...
# Streamlit UI
st.title("🎥 YouTube Summarizer (Transcripts + Whisper + Metadata)")

# Input YouTube URL
video_url = st.text_input("Enter YouTube Video URL:")

# Function to download audio in M4A format
def download_audio(video_id, output_dir="audio"):
    try:
        os.makedirs(output_dir, exist_ok=True)
        timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
...
def fetch_metadata_youtube_api(video_id):
    try:
        ...
        if not response.get('items'):
            st.warning("No metadata found for the video.")
            return None, None
        title = response['items'][0]['snippet']['title']
        description = response['items'][0]['snippet']['description']
        return title, description
...
def get_summary(text, source):
    with st.spinner(f"Summarizing using Gemini ({source})..."):
        return gemini_api_function(text)
...
if video_url:
    try:
        ...
        if transcript:
            source = "Transcript"
            summary = get_summary(transcript, source)
        else:
            whisper_text = get_audio_transcription(video_id)
            if whisper_text:
                source = "Whisper Transcription"
                summary = get_summary(whisper_text, source)
            else:
                title, description = fetch_metadata_youtube_api(video_id)
                if not title and not description:
                    st.warning("Could not extract any content to summarize.")
                else:
                    metadata = f"Title: {title}\n\nDescription: {description}"
                    source = "Video Metadata"
                    summary = get_summary(metadata, source)

        if summary:
            st.subheader(f"Summary from {source}")
            st.markdown(summary)
